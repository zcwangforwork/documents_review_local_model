...
"""
    ...
    def __init__(self, vector_store, api_key: str, api_url: str, model: str = "qwen3.5:122b"):
    ...
    async def search(self, query: str, top_k: int = 5) -> List[Dict[str, Any]]:
        ...
        Returns:
            相关文档片段列表，每项包含 text, source, chunk_id
        """
        return await self._retrieve_for_section(query, n_results=top_k)
    ...
    async def _audit_single_section(
        ...
        async with semaphore:
            try:
                # 检索相关知识库内容
                relevant_docs = await self._retrieve_for_section(section_content, n_results=8)
                ...
                if relevant_docs:
                    for i, doc in enumerate(relevant_docs, 1):
                        ...
                        context_parts.append(doc['text'][:3000])
                else:
                    context_parts.append("[未在知识库中找到直接相关的参考内容，请基于标准原文进行审核]")

                user_content = '\n'.join(context_parts)

                # 调用 LLM（Phase 1: max_tokens 6000→8000 适应更细粒度+更全内容+更长Rubric输出）
    ...
    async def analyze_document(
        ...
        # ===== 第二轮：扁平化为审核单元（四级小节为单位，更细粒度）=====
        audit_units = flatten_to_audit_units(outline, audit_granularity=4)
        logger.info(f"扁平化后共 {len(audit_units)} 个审核单元（四级小节粒度）")
        ...
        MAX_SECTION_ANSWER_CHARS = 5000  # 每个小节详情在最终报告中的最大字符数（放宽以适应细粒度审核）
...
def create_rag_retriever(vector_store, api_key: str, api_url: str, model: str = "qwen3.5:122b") -> RAGRetriever:
    """
    工厂函数：创建 RAG 检索器实例
